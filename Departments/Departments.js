...
function renderCards() {
  ...
  document.getElementById("deptFiles").innerText = filtered.reduce((acc, s) => acc + (parseInt(s.file_count) || 0), 0);
  document.getElementById("deptStorage").innerText = formatBytes(filtered.reduce((acc, s) => acc + (parseInt(s.storage_bytes) || 0), 0));
  ...
  paginatedItems.forEach((s) => {
    const img = s.profile_picture || `https://ui-avatars.com/api/?name=${s.first_name}+${s.last_name}`;
    container.innerHTML += `
            <div class="staff-card">
                <div class="staff-avatar"><img src="${img}" alt="profile" onerror="this.src='../uploads/profiles/default.png'"></div>
                <div style="flex:2">
                    <strong style="font-size: 1.1rem; color: var(--text-main);">${s.first_name} ${s.last_name}</strong>
                    <div><span class="badge ${s.role.toLowerCase()}">${s.role}</span></div>
                </div>
                <div style="flex:1; text-align:center; border-left: 1px solid var(--border);">
                    <div style="display:flex; align-items:center; justify-content:center; gap:8px;">${ICONS.files} <strong>${s.file_count || 0}</strong></div>
                    <small style="font-size:0.65rem; color:var(--text-dim); text-transform:uppercase;">Files Uploaded</small>
                </div>
                <div style="flex:1; text-align:center; border-left: 1px solid var(--border);">
                    <div style="display:flex; align-items:center; justify-content:center; gap:8px;">${ICONS.storage} <strong>${formatBytes(s.storage_bytes)}</strong></div>
                    <small style="font-size:0.65rem; color:var(--text-dim); text-transform:uppercase;">Storage Consumed</small>
                </div>
...
/* ======================================
   ...
   function openAddModal() {
    ...
    document.getElementById("preview").innerHTML = `<img src="../uploads/profiles" style="width:100%;height:100%;object-fit:cover;">`;
