...
async function restoreItem(id, type) {
    ...
    try {
        ...
        if (data.success) { fetchTrash(); alert("Restored."); }
...
async function deletePermanent(id, type) {
    ...
    try {
        ...
        const text = await res.text();
        const data = text ? JSON.parse(text) : { success: true };
        if (data.success) { fetchTrash(); alert("Deleted."); }
...
async function bulkAction(action) {
    ...
    const endpoint = action === 'restore' ? 'restore_item.php' : 'permanent_delete.php';
    try {
        const res = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items }) });
        const text = await res.text();
        const data = text ? JSON.parse(text) : { success: true };
        if (data.success) { selectedItems.clear(); fetchTrash(); alert(`Bulk ${action} successful.`); }
    } catch (e) { console.error(e); }
}
